/// Prints the difference of lines between the lines vecs ```seq_1``` & ```seq_2``` using the grid ```grid``` generated by lcs previously. ```i``` is the lenght of ```seq_1``` and ```j``` the lenght of ```seq_2```
